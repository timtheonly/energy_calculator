...
class TestHdfParser(unittest.TestCase):
    ...
    def test_extra_data(self):
        fake_data = """Read Value,Read Date and End Time,Read Type\n0.010,01-01-2026 00:00,Active Import Interval (kWh)\n0.010,01-01-2026 00:30,Active Import Interval (kWh)\n0.010,01-01-2026 01:00,Active Import Interval (kWh)\n0.010,02-01-2026 01:00,Active Import Interval (kWh)\n"""
        with patch(
            "energy_calculator.hdf_parser.open", mock_open(read_data=fake_data)
        ) as m:
            self.parser.parse(filename="test.csv")
            m.assert_called_once_with("test.csv", "r+")
    ...
    def test_export(self):
        fake_data = """Read Value,Read Date and End Time,Read Type\n0.010,01-01-2026 00:00,Active Import Interval (kWh)\n0.010,01-01-2026 00:30,Active Import Interval (kWh)\n0.010,01-01-2026 01:00,Active Import Interval (kWh)\n0.010,01-01-2026 01:00,Active Export Interval (kWh)\n"""
        with patch(
            "energy_calculator.hdf_parser.open", mock_open(read_data=fake_data)
        ) as m:
            self.parser.parse(filename="test.csv")
        ...
        assert self.parser.end == datetime(year=2026, month=1, day=1, hour=1, minute=0)
        assert self.parser.total_import == 0.030
        assert self.parser.total_export == 0.010

    def test_non_existent_file(self):
        with self.assertRaises(ValueError):
            self.parser.parse(filename="blah.csv")
    ...
    def test_no_start_or_end(self):
        parser = HDFParser()
        fake_file = io.StringIO(self.fake_data)
        parser.parse(file=fake_file)
